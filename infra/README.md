...
- **Terraform pipeline (`terraform.yml`):** separate from app deploy, separate blast radius. `plan` runs automatically (PR/push touching `infra/terraform/**`, posts the plan as a PR comment); `apply` only runs via manual `workflow_dispatch` — nothing auto-applies infra changes. Uses a third SA, `terraform-deployer` (roles: `editor`, `run.admin`, `secretmanager.admin`, `iam.serviceAccountUser` — matches what the human operator has used all along), kept separate from `github-actions-deployer` so "ship code" and "reshape infrastructure" stay genuinely different privilege levels.
- **Image tag is permanently protected from Terraform drift:** `modules/deployment/modules/cloud-run-service/main.tf` has `lifecycle { ignore_changes = [...] }` on the container `image` field. CI's `gcloud run deploy` is the sole owner of what's live; `terraform apply` — automatic or manual, now or in six months — can never revert it, regardless of what's in `terraform.tfvars`. Verified live by setting `image_tag` to a nonsense value and confirming `terraform plan` still reported "No changes."
