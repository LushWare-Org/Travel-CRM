...
describe('geminiClient', () => {
  ...
  it('retries with a larger token budget when Gemini reports MAX_TOKENS, and succeeds', async () => {
    mockGenerateContent
      .mockResolvedValueOnce({ text: '{"days":[{}]}', candidates: [{ finishReason: 'MAX_TOKENS' }] })
      .mockResolvedValueOnce({ text: '{"days":[{},{},{}]}', candidates: [{ finishReason: 'STOP' }] });
    const { generateStructured } = await import('../geminiClient.js');
  ...
  it('gives up with a clear error after repeated truncation', async () => {
    mockGenerateContent.mockResolvedValue({ text: '{"days":[]}', candidates: [{ finishReason: 'MAX_TOKENS' }] });
    const { generateStructured } = await import('../geminiClient.js');
