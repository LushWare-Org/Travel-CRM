...
test.describe('Role-based navigation', () => {
  test('superAdmin sees Career but not Leads', async ({ superAdminPage }) => {
    await expect(superAdminPage.getByRole('button', { name: 'Career' })).toBeVisible();
    await expect(superAdminPage.getByRole('button', { name: 'Leads' })).toHaveCount(0);
  });

  test('admin sees Settings and Leads but not Career', async ({ adminPage }) => {
    await expect(adminPage.getByRole('button', { name: 'Settings' })).toBeVisible();
    await expect(adminPage.getByRole('button', { name: 'Leads' })).toBeVisible();
    await expect(adminPage.getByRole('button', { name: 'Career' })).toHaveCount(0);
  });

  test('salesRep sees Leads but not Settings or Career', async ({ salesRepPage }) => {
    await expect(salesRepPage.getByRole('button', { name: 'Leads' })).toBeVisible();
