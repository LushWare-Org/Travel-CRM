...
- **Sidebar/nav visual treatment** is not yet designed beyond the `sidebar-*` tokens already present in `src/index.css` (mirroring `card`/`border`/`primary`) - the actual `Sidebar.jsx` component doesn't migrate until Phase 2, and its concrete layout/interaction spec belongs in this file once that happens.
