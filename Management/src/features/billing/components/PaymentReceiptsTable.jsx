...
import { Eye, Edit, Trash2, Download, Send, CheckCircle, DollarSign } from 'lucide-react';

const PaymentReceiptsTable = ({ 
  receipts, 
  onView, 
  onEdit, 
  onDelete, 
  ...
  const getStatusConfig = (status) => {
    ...
    return configs[status] || configs['paid-in-advance'];
  };
  ...
  if (receipts.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
        ...
        <p className="mt-1 text-sm text-gray-500">Payment receipts will appear here when customers make payments.</p>
      </div>
    );
  }
                ...
                <tr key={receipt.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm">
                      ...
                      <div className="text-gray-500 text-xs">Lead ID: {receipt.leadId}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{receipt.invoiceId}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-gray-500">Ref: {receipt.transactionId}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{formatDate(receipt.paymentDate)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
