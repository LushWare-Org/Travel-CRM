...
### Added
- **An AI voice agent can answer the phone.** A number published through Retell is answered by an agent that greets the caller, recognises a repeat customer from their number, answers questions about their trips and the packages on offer, prepares itinerary and price changes for a rep to approve in the lead's AI tab, and resends a document the customer was already sent. It never speaks a figure it calculated itself, never speaks a flight fare, and never books anything — a rep approves every change, and the whole call is stored with its transcript, summary, sentiment, recording link and the actions the agent took. When it promises a callback the sales team is emailed. Its script lives in `docs/voice-agent-prompt.md`, not in the Retell dashboard, and is pushed live by `Services/setup-voice-agent.mjs`.
- The marketing Landing page now deploys to Firebase Hosting as its own site (`lush-ware-landing-dev`). It ships both by hand (`scripts/deploy-landing.sh`) and automatically from CI's `deploy-hosting` job on every push to `main`. Its links to the Client and Management portals are set per environment at build time, so a dev deploy no longer points visitors at production.
...
- Ask the assistant about what is on your screen — how many results are showing, how many match the filters you have on — and it answers from the page's own count, with those filters shown as chips. It knows which page you are on and stops answering as if you were still on the last one.
- The assistant can fill a form for you: the contact form, the booking and review dialogs, and the job application. It writes only the fields you actually gave it, marks them "Filled by assistant" until you edit them, and asks before replacing text you typed yourself. It never submits anything — the button stays yours.
